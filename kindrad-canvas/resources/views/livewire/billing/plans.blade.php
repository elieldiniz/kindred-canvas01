<div class="flex flex-col gap-section p-margin-page">
    ...
    <header class="py-8 pb-2">
        ...
        <p class="mt-3 max-w-xl text-base text-white/50 leading-relaxed">
            {{ __('Cada plano concede créditos mensais automáticos. Você pode trocar ou cancelar a qualquer momento.') }}
        </p>
    </header>
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3" data-test="plans-grid">
        @forelse ($plans as $loop_plan)
            ...
            >
                ...
                <div class="mb-5 flex items-end gap-1">
                    ...
                    <span class="mb-1 text-sm text-white/40">/ {{ $loop_plan->interval->slug === 'year' ? 'ano' : 'mês' }}</span>
                </div>
                ...
                <div class="mb-6 flex items-center gap-2">
                    ...
                    <p class="text-sm text-white/60" data-test="plan-credits-{{ $loop_plan->slug }}">
                        ...
                        {{ __('créditos por') }} {{ $loop_plan->interval->slug === 'year' ? __('ano') : __('mês') }}
                    </p>
                </div>
                ...
                <div class="mt-auto">
                    ...
                    >
                        {{ __('Assinar :name', ['name' => $loop_plan->name]) }}
                    </button>
                </div>
            </article>
        @empty
            <div class="col-span-3 flex flex-col items-center gap-4 rounded-2xl border border-white/5 bg-surface-container/40 p-16 text-center">
                ...
                <p class="text-white/50" data-test="plans-empty">{{ __('Nenhum plano disponível no momento.') }}</p>
            </div>
        @endforelse
    </div>
